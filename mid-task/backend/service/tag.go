package service

import (
	"github.com/jmoiron/sqlx"
	"github.com/pkg/errors"
	"github.com/yutaro1031/treasure-app/mid-task/backend/dbutil"

	"github.com/yutaro1031/treasure-app/mid-task/backend/model"
	"github.com/yutaro1031/treasure-app/mid-task/backend/repository"
)

type Tag struct {
	db *sqlx.DB
}

func NewTag(db *sqlx.DB) *Tag {
	return &Tag{db}
}

func (a *Tag) Update(id int64, newTag *model.Tag) error {
	_, err := repository.FindTag(a.db, id)
	if err != nil {
		return errors.Wrap(err, "failed find tag")
	}

	if err := dbutil.TXHandler(a.db, func(tx *sqlx.Tx) error {
		_, err := repository.UpdateTag(tx, id, newTag)
		if err != nil {
			return err
		}
		if err := tx.Commit(); err != nil {
			return err
		}
		return err
	}); err != nil {
		return errors.Wrap(err, "failed tag update transaction")
	}
	return nil
}

func (a *Tag) Destroy(id int64) error {
	_, err := repository.FindTag(a.db, id)
	if err != nil {
		return errors.Wrap(err, "failed find tag")
	}

	if err := dbutil.TXHandler(a.db, func(tx *sqlx.Tx) error {
		_, err := repository.DestroyTag(tx, id)
		if err != nil {
			return err
		}
		if err := tx.Commit(); err != nil {
			return err
		}
		return err
	}); err != nil {
		return errors.Wrap(err, "failed tag delete transaction")
	}
	return nil
}

func (a *Tag) Create(newTag *model.Tag) (int64, error) {
	var createdId int64
	if err := dbutil.TXHandler(a.db, func(tx *sqlx.Tx) error {
		result, err := repository.CreateTag(tx, newTag)
		if err != nil {
			return err
		}
		if err := tx.Commit(); err != nil {
			return err
		}
		id, err := result.LastInsertId()
		if err != nil {
			return err
		}
		createdId = id
		return err
	}); err != nil {
		return 0, errors.Wrap(err, "failed tag insert transaction")
	}
	return createdId, nil
}
